...
export default function UserInfoSidebar() {
  ...
  return (
    <div
      className={cn(
        'max-w-[320px] min-w-[250px] bg-foreground overflow-hidden h-screen hidden md:flex flex-col border-r border-borderColor duration-500',
        { 'w-[15rem] sm:w-[20rem]': isOpen, 'w-[5rem]  sm:min-w-[5rem]': !isOpen }
      )}
    >
      <div className="p-4">
        ...
        <div className={cn('text-sm w-full justify-between', { hidden: !isOpen, flex: isOpen })}>
          <div className="px-2 text-center">
            <div className="font-semibold">{userInfo.followingCount}</div>
            <div className="text-whity font-thin text-sm">Following</div>
          </div>
          <div className="px-2 text-center">
            <div className="font-semibold">{userInfo.followerCount}</div>
